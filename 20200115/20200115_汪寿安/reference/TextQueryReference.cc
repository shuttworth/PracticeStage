...
public:
    ...
    string text_line(int line) const {
        if (line < (int)lines_of_text.size())
...
private:
    ...
    //建立单词 和行号组成的键值对映射关系
    void build_map() {
        for (int line_no = 0; line_no != (int)lines_of_text.size(); line_no++) {
...
int main() {
    TextQuery file = build_textfile("china_daily.txt");
