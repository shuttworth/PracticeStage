...
void test0()
{
	...
	display(cities);
	cout << cities[1] << endl; //通过key找值，打印结果是北京
	//map支持下标访问运算符；
	//
	//如果关键字key存在，则直接对应的value;见上
	//如果关键字key不存在，则直接创建一个;见下
	cout << cities[6] << endl;
	cout << endl;
	display(cities);
	cities[6] = "武汉";
...
void test1()
{
	...
	map<string, string> cities{
		...
		std::make_pair("22", "上海"), //构造 std::pair 对象，从参数类型推导目标类型
		std::make_pair("33", "广州"),
		std::make_pair("44", "深圳"),
		std::make_pair("55", "杭州"),
		std::make_pair("22", "武汉"),
	...
	cout << endl;
	display(cities);
	cities["66"] = "武汉";//存在即修改，不存在则创建
    //下标访问运算符还可以修改数据, 时间复杂度O(logN)
	cout << endl << "修改完value之后:" << endl;
	display(cities);

	//只查找,count\find都可以实现查找
	cout << cities.count("11") << endl; //返回的是11的数目
